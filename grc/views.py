...
def Portal(request):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            response.set_cookie(
                "groups", str(groups_selected)[1:-1].replace(", ", "|"))
...
def Review(request):
    def get_questions(name_en):
        ...
        questions = Question.objects.filter(
            group__name_en=name_en).order_by("created")
        return questions
    ...
    try:
        groups = eval(
            ("[" + request.COOKIES.get("groups") + "]").replace("|", ","))
        ...
        if order:
            ...
            if qseq == len(questions):
                ...
                response = render_to_response(
                    "review.html", {"question": questions[qseq]})
            ...
            response = render_to_response(
                "review.html", {"question": all_questions[qseq_to_render]})
            response.set_cookie(
                "chosen", ("{" + str(chosen)[5:-2] + "}").replace(", ", "|"))
